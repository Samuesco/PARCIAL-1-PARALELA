...
struct Image {
    ...
    int* data;       // width*height*channels
};
...
bool read_image(const char* name_arc, Image* im) {
    FILE* f = std::fopen(name_arc, "r");
    if (!f) {
        std::fprintf(stderr, "Error abriendo '%s'\n", name_arc);
        return false;
    }
    ...
    for (long i = 0; i < total; ++i) {
        ...
        if (std::fscanf(f, "%d", &v) != 1) {
            std::free(im->data); im->data = nullptr; std::fclose(f);
            return false;
        }
...
bool write_image(const char* name_arch, const Image* im) {
    FILE* f = std::fopen(name_arch, "w");
    if (!f) {
...
void apply_kernel3x3(const Image* in, Image* out, const float K[3][3], bool normalize) {
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            else { // C==3: procesar R,G,B juntos
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 2; i < argc; ++i) if (std::strcmp(argv[i], "--f") == 0 && i + 1 < argc) filtro = argv[++i];
    if (!filtro) { usage(argv[0]); return 1; }
    ...
    if (std::strcmp(filtro, "blur") == 0) {
        apply_kernel3x3(&in, &out, K_blur, true);
    }
    else if (std::strcmp(filtro, "laplace") == 0) {
        apply_kernel3x3(&in, &out, K_laplace, false);
    }
    else if (std::strcmp(filtro, "sharpen") == 0) {
        apply_kernel3x3(&in, &out, K_sharpen, false);
    }
    else {
    ...
    build_unique(outp, MAX_PATH_LEN, filtro, &in, "seq");
