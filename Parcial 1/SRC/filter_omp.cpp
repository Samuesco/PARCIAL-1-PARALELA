...
void kernel3x3_omp(const Image* in, Image* out, const float K[3][3], bool normalize) {
...
int main(int argc, char* argv[]) {
    ...
    if (std::strcmp(filtro, "blur") == 0){
        kernel3x3_omp(&in, &out, K_blur, true);
    }else if (std::strcmp(filtro, "laplace") == 0){
        kernel3x3_omp(&in, &out, K_laplace, false);
    }else if (std::strcmp(filtro, "sharpen") == 0){
        kernel3x3_omp(&in, &out, K_sharpen, false);
    }else {
    ...
    ensure_output_dir(); char outp[MAX_PATH_LEN];
    build_unique(outp, MAX_PATH_LEN, filtro, &in, "omp");
